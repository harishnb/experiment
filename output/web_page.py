...
def hours_minute_range():
    minutes_series = []
    hour_series = []
    time_series = []

    for i in range (0,24):
        hour_series.append(i)

    for i in range (0,60):
        minutes_series.append(i)

    for h in hour_series:
        for m in minutes_series:
            if h < 10:
                h_tmp = "0"+str(h)
            else:
                h_tmp = str(h)
            if m < 10:
                ...
                m_tmp = str(m)
            time_series.append(str(h_tmp) +":"+ str(m_tmp ))
...
#*************************************** Graph of Average uptime***********************************
        body_lines.append('<td> </td><td></td><td align="right" ><object type="image/svg+xml" data="static/images/time_series_chart.svg" style="margin-top:10px; margin-left:100px;"> Your browser does not support SVG </object></td>')
        # body_lines.append('</tr></table>')
        # body_lines.append('<td>')
        # body_lines.append('<p>====================================================================================================================================================================================</p>')
...
#####################################Active session line chart #####
    line_chart = pygal.Line(width=400, height=400,explicit_size=True, title=title,x_label_rotation=90)
    ...
    for idx1 in list(range(0,len(data_time_spent))):
        ...
        var_start_time_series.append(float(datetime.utcfromtimestamp(int(tmp_start_time)).strftime('%H.%M')))
        var_end_time_series.append(float(datetime.utcfromtimestamp(int(data_time_spent.Date_Time[idx1])).strftime('%H.%M')))
